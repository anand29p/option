...
def volume_ratio(df: pd.DataFrame, period: int = 20) -> pd.Series:
    """Volume ratio: current / rolling mean. >1.5 = above-average volume."""
...
def is_trending(df: pd.DataFrame, ema_fast: int = 9, ema_slow: int = 21, slope_threshold: float = 0.05) -> str:
    """Detect trend using dual EMA slope. Returns: UP | DOWN | SIDEWAYS"""
...
def pivot_points(df_prev_day: pd.DataFrame) -> dict:
    ...
    pp = (h + l + c) / 3
    return {
        "PP": round(pp, 2), "R1": round(2 * pp - l, 2),
        "R2": round(pp + (h - l), 2), "R3": round(h + 2 * (pp - l), 2),
