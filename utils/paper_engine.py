...
class Position:
    ...
    target:         float
    strategy:       str
    security_id:    str = ""  # Dhan security ID (used for LTP fetch)
    status:         str = "OPEN"
...
class PaperEngine:
    ...
    def enter_trade(
        ...
        _tmp = SimpleNamespace(symbol=symbol, security_id=security_id)
        ltp = self._fetch_ltp(_tmp)
    ...
    def _fetch_ltp(self, pos) -> Optional[float]:
        """Fetch current LTP via Dhan security_id (fast path) or symbol fallback."""
        try:
            if hasattr(pos, 'security_id') and pos.security_id:
                data = self.kite.get_ltp([pos.security_id], exchange="NSE_FNO")
                return data.get(pos.security_id)
            # Fallback: try symbol-based LTP (older code path)
            symbol = pos.symbol if hasattr(pos, 'symbol') else pos
            data = self.kite.get_ltp([symbol])
    ...
    def open_positions_summary(self) -> list[dict]:
        ...
        for pos in self.open_trades.values():
            ltp = self._fetch_ltp(pos) or pos.entry_price
